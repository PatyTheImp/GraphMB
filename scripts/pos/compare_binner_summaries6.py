...
def plot_combined(output_file):
    ...
    axes[0].barh(y, mq_gsa, height=bar_height, left=hq_gsa, label='MQ (GSA)', color=color_mq_gsa)
    axes[0].barh(y, hq_gsa, height=bar_height, label='HQ (GSA)', color=color_hq_gsa)

    # Stacked bars for MA
    axes[0].barh(y + 1, mq_ma, height=bar_height, left=hq_ma, label='MQ (MA)', color=color_mq_ma)
    axes[0].barh(y + 1, hq_ma, height=bar_height, label='HQ (MA)', color=color_hq_ma)

    # Y-axis labels
    yticks = y + 0.5
    yticklabels = [f"{tool}" for tool in tools]
    axes[0].set_yticks(yticks)
    axes[0].set_yticklabels(yticklabels, fontsize=label_size)
    axes[0].set_ylabel('Marine', fontsize=label_size, fontweight='bold')
    ...
    for i in range(1,3):
        summary_df, _ = load_summaries_from_folders(f'summaries{i-1}')
